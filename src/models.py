...
class DecodeBlock(nn.Module):
    def __init__(self, in_channel, skip_channel, out_channel, upsample):
        super().__init__()

        if skip_channel is not None:
            self._skip_conv1x1 = conv1x1(skip_channel, in_channel)
        else:
            self._skip_conv1x1 = None
    ...
    def forward(self, inputs, skip_inputs=None):
        if skip_inputs is not None:
            skip_inputs = self._skip_conv1x1(skip_inputs)
